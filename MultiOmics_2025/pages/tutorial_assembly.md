...
Before we can start We will have to use `ssh` (Secure SHell) to connect to `dardel`, use you ssh-client of choice. To start, we have to log into the login node. Hopefully you set it all up with the keys.
...
Checking on data on the cluster is kinda easy but can be different to what you are used to. On you computer you might be used to click on files, and they will be opened by whatever program you OS thinks is appropriate. This does not work on the cluster, there is no clicking.

However, most files you might want to look at are ASCII/text-files. Note that not onlye `.txt`-files are text files, many files you will generate are text files with other extentions, like `.fna` or `.sam` or even `.html`. All text files can be opened with text-`viewers`. My usual one is `less`, but there are many more.
...
You should see some structure text/data. You can now go up and down with usual keys (arrows, page-up, page-down, or the spacebar). And you can exit with `q`.
...
Shoudl show you some stuff too, but it might be somewhat too cryptic. So let's actually copy the file over to your own computer to open it with a web-browser to make sense of it. My way of choice is using scp, your milage might vary.
...
Let's find out who is in here, we will directly taxonomically annotate the reads, e.g. look at each read individualy and try to guess what organism it comes from. We will used kraken2 for this:
...
# if it's the first time you you run krona, you will want to run this
...
From the outputs find the percentage of classified reads, and the name of the most abundant genus and add it to the google-sheet. ** Once your wrote the stuff into the google-sheet, check in on the etherpad **
